...
async function procesarMensaje(mensaje) {
    ...
    // Precios - Cotillón
    if (mensaje.match(/cotillon|globo|vela|cumpleaños|cumpleanos|fiesta|piñata|decoracion/)) {
    ...
    // Impresiones
    if (mensaje.match(/fotocopia|impresi|imprim|sublim|remera|taza|edicion|diseño|diseño/)) {
...
function buscarPreciosCategoria(categoria, mensaje) {
    ...
    for (const [subcategoria, productos] of Object.entries(datos)) {
        for (const [nombre, info] of Object.entries(productos)) {
            ...
            const nombreFormateado = nombre.replace(/_/g, ' ').replace(/\, . +}/g, l => l.toUpperCase());
...
console.log('📱 Escanea el QR code que aparecerá en la consola');